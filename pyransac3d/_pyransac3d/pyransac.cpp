...
#define M_PI 3.14159265358979323846
	...
	int ransac_line(std::vector<std::vector<double>> &data, PyObject *PyInliers, PyObject *PyVersors, PyObject *PyPoints, int number_it, double min_dist, int min_inlier)
	{
		...
		for (int m = 0; m < parcial.size(); m++)
			PyList_Append(PyInliers, PyLong_FromLong((long int)parcial[m]));
		for (loop = 0; loop < 3; loop++)
		{
	...
	PyMODINIT_FUNC PyInit__pyransac3d(void)
	{
		import_array();
		return PyModule_Create(&_pyransac3d);
	}
}
